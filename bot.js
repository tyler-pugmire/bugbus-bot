...
bot.on('message', function (user, userID, channelID, message, evt) {
  ...
  if (message.substring(0, 1) == '!') {
    ...
    var mod = bot.servers[bot.channels[channelID].guild_id].roles.find("name", "mod");
    var bestmod = bot.servers[bot.channels[channelID].guild_id].roles.find("name", "!bestmod");
    ...
    switch(cmd) {
      ...
      case 'addcmd':
        if(member.roles.has(mod.id) || member.roles.has(bestmod.id)){
