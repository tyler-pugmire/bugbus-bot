...
const disambiguation = require('discord.js-commando').disambiguation;
const fs = require('fs');
	...
	async run(msg, args) { // eslint-disable-line complexity
		...
		if(args.command && !showAll) {
            ...
            messages.push(await msg.channel.send({embed: {
              ...
              description: `${msg.author} Sent you a DM with information.`
            }}));
          }
				} catch(err) {
					messages.push(await msg.channel.send({embed: { 
            color : parseInt(globals.errorColor),
            description:`${msg.author} Unable to send you the help DM. You probably have DMs disabled.`
          }}));
				}
				return messages;
